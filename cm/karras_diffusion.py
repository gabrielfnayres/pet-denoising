...
class KarrasDenoiser:
    ...
    def training_losses(self, model, x_start,condition, sigmas, model_kwargs=None, noise=None):
        ...
        model_output, denoised = self.denoise(model, x_t,condition, sigmas, **model_kwargs)
        ...
        # terms["xs_mse"] = mean_flat(denoised, x_start)
        if "vb" in terms:
            terms["loss"] = terms["mse"] + terms["vb"]
        else:
            terms["loss"] = terms["xs_mse"]
    ...
    def consistency_losses(
        ...
        def denoise_fn(x,condition, t):
            return self.denoise(model, x,condition, t, **model_kwargs)[1]

        if target_model:

            @th.no_grad()
            def target_denoise_fn(x,condition, t):
                return self.denoise(target_model, x,condition, t, **model_kwargs)[1]
        ...
        if teacher_model:
            ...
            def teacher_denoise_fn(x, t):
                return teacher_diffusion.denoise(teacher_model, x,condition, t, **model_kwargs)[1]
        ...
        distiller = denoise_fn(x_t,condition, t)
        ...
        distiller_target = target_denoise_fn(x_t2,condition, t2)
        ...
        if self.loss_norm == "l1":
            ...
            # loss = mean_flat(distiller,distiller_target) * weights
    ...
    def denoise(self, model, x_t,condition, sigmas):
        ...
        model_input = th.cat([x_t,condition],dim=1)
        model_output = model(c_in * model_input, rescaled_t)
...
def karras_sample(
    ...
    def denoiser(x_t, condition, sigma): 
        _, denoised = diffusion.denoise(model, x_t,condition, sigma)
        if clip_denoised:
...
def sample_heun(
    ...
    for i in indices:
        ...
        # model_input = th.cat([x,condition],dim=1)
        denoised = denoiser(x, condition, sigma_hat * s_in)
        ...
        if sigmas[i + 1] == 0:
            ...
            # Heun's method
            x_2 = x + d * dt
            denoised_2 = denoiser(x_2, condition, sigmas[i + 1] * s_in)
...
def sample_euler(
    ...
    for i in indices:
        sigma = sigmas[i]
        denoised = denoiser(x,condition, sigma * s_in)
...
def stochastic_iterative_sampler(
    ...
    for i in range(len(ts) - 1):
        ...
        x0 = distiller(x,condition,t * s_in)
